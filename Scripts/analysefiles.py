...
def ECFsc(phi):
    ...
    modelist, ftl, nstep, winlist = ECF_simple(
        x_hat,
        nstepmax=nstepmax,
        nwindow=nwindow,
    ...
    np.save(
        outecf + "ECF_phi_{:.3f}_{:.3f}_{:d}.npy".format(t1, t2, nwindow),
        np.vstack((modelist, ftl)),
    )
    ...
    for index, row in df.iterrows():
        ...
        if True:
            ...
            if True:
                ...
                plt.hist(
                    X, 400, color="lightseagreen", label="Raw histogram", density=True
                )
                plt.plot(xar, fit)
