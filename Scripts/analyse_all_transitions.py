...
# Loop through the rows and read columns using the keys in the first row
    for index, row in df.iterrows():
        ...
        for f in fl:
            ...
            all_t,all_t_up,all_t_down = summarize_transition_stats(peaks,timess, do_double =0)
...
#                    if (all_t[i,j] ==0):
#                        print(timess[i,j])
                    plt.errorbar([iind],all_t[i,j],yerr=[[all_t[i,j]-all_t_down[i,j]],[all_t_up[i,j]-all_t[i,j]]],color=color_cycle[iex%len(color_cycle)],marker = markers[im%len(markers)])
                    iind += 1
                    alth.append(all_t[i,j])

                if len(timess[j,i])>10:
                    plt.errorbar([iind],[all_t[j,i]],yerr=[[all_t[j,i]-all_t_down[j,i]],[all_t_up[j,i]-all_t[j,i]]],color=color_cycle[iex%len(color_cycle)],marker = markers[im%len(markers)])
                    iind += 1
                    alth.append(all_t[i,j])
