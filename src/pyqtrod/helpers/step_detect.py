...
def t_scan(L, window=1e3, num_workers=-1):
    ...
    if num_workers == 1:
        ...
        results = [
            pool.apply_async(_t_scan_drone, args=(L, n_cols, frame, window))
            for frame in frames
        ]
    ...
    t_stat = np.concatenate(
        (np.zeros(window), t_stat.transpose().ravel(order="C"), np.zeros(size % window))
    )
...
def filter_step(datac, n=5, t=0.001, windowavsteps=10):
    data = np.copy(datac)
    p2 = mz_fwt(data, n=n)
    L = find_steps(np.abs(p2), t)
    L.insert(0, 0)
    filtered = np.copy(data)
    sstep = []
    indstep = []
    for j in range(len(L) - 1):
        filtered[L[j] : L[j + 1]] = np.average(data[L[j] : L[j + 1]]) * np.ones(
            L[j + 1] - L[j]
        )
        if j != 0:
            sstep.append(filtered[L[j]] - filtered[L[j - 1]])
            indstep.append(L[j])
    sstep = np.asarray(sstep)
    indstep = np.asarray(indstep)
    step_sizes, step_error = get_step_sizes(data, L, window=windowavsteps)
    return filtered, p2, step_sizes
