...
def get_ruptures(
    xt, ar, min_size=10, penalty_value=20, show=1, showstepvalue=0, showstep=1, dpi=100
):
...
def step_properties(xbound, m):
    ...
    dwelltime = np.diff(
        xbound
    )  # first dwell time corresponds to time before second step_size #last dwell time correspond to nothing
    stepsize = stepsize[1:]
