...
def worker1(c1, c2, f, func, q, **kwargs):
    ...
    x, y = func(
        phi, c1 / f.get_freq(), c2 / f.get_freq(), **kwargs
    )  # time given by the freq
    ...
    **kwargs,
    ...
    if chunks is None:
        if overlap is None:
            ...
            chunksstart = np.arange(
                tstart * freq, tmax * freq - chunk_size, overlap * freq
            )
            chunksstop = chunksstart + chunk_size * freq
    ...
    for i in range(len(chunksstart)):
        job = pool.apply_async(
            worker1,
            args=(chunksstart[i], chunksstop[i], inst, func, q),
            kwds=kwargs,
    ...
    for i in range(len(chunksstart)):
        r = jobs[i].get()
        print(
            "Analyzing Chunk [" + str(chunksstart[i]) + "," + str(chunksstop[i]) + "]"
        )
        if r is not None:
            x, y = r
            writef(outf, x, y)
