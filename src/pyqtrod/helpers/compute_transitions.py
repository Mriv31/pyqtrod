from .step_helpers import get_ruptures_mm  # or whatever name you want.
import numpy as np
...
def compute_transitions(output, xi, zi, xar, peaks, pen=0.1, min_segment_size=5):
    ...
    for i in range(1, len(indlih) - 1):  # I don't consider the first transition
        transitionarh[indlih[i]][indlih[i + 1]].append(
            xboundh[i]
        )  # contains  all transitions and times
    ...
    for i in range(1, len(indlih)):
        for j in range(len(peaks)):
            if rawtrh[indlih[i], j][-1].size == 1:
                rawtrh[indlih[i], j][-1] = np.arange(
                    xboundrawh[i - 1], xboundrawh[i], dtype=int
                )

            else:
                rawtrh[indlih[i], j][-1] = np.hstack(
                    (
                        rawtrh[indlih[i], j][-1],
                        np.arange(xboundrawh[i - 1], xboundrawh[i], dtype=int),
                    )
                )
            meanzh[indlih[i], j][-1].append(
                np.mean(zi[xboundrawh[i - 1] : xboundrawh[i]]) % (2 * np.pi)
            )
    ...
    np.savez(
        ...
        meanz=meanzh,
        timess=timessh,
        durations=durationsh,
        rawtr=rawtrh,
        peaks=peaks,
        xar=xar,
        xbound=xboundh,
        indli=indlih,
        xboundraw=xboundrawh,
        m=mh,
    )
