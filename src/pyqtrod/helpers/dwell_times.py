...
def dwell_times(
    xt, phiuc, min_a, max_a, median=0, medianparam=100, plot=0, plotresult=0
):
    if median:
    ...
    phirh = phiuh % (2 * np.pi)
    ind = np.where(
        np.logical_and(phirh > min_a, phirh < max_a)
    )  # indices of frames at a given index calue
    arr = xt[ind]  # times where angle is between the value above
    n, p = np.histogram(arr, bins=np.arange(xt.min(), xt.max(), 0.0008))
    # histogram of the times, the larger the number of bins the more you may separate times
    arr = n

    sub_arrays = np.split(
        arr, np.where(arr == 0)[0] + 1
    )  # splitting the histogram into subhistograms separated by zero (will be affected by the number of bins avove)
    if len(sub_arrays[-1]) == 0:
        sub_arrays = sub_arrays[:-1]
    result = [
        np.add.reduce(sub_array) for sub_array in sub_arrays if len(sub_array) > 1
    ]  # sum of consecutive bins not separated by zeros
