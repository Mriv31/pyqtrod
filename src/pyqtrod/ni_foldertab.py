...
import glob
import numpy as np
from .pyqtfunc import anisotropies, PSD
...
class NIFolderTab(QtWidgets.QMainWindow):
    def __init__(self, NIfolder, png_instance=None):
        ...
        self.scrolled.setHorizontalScrollBarPolicy(
    ...
    def displayNIfileSum(self, file, w):
        ...
        w.addScatterPlot(
            x=x[:40000],
            y=y[:40000],
            xtitle="Anisotropy 0/90",
            ytitle="Anisotropy 45/135",
        )
        ...
        w.addPlot(
            x=x[ind], y=y[ind], logx=0, logy=1, xtitle="Freq (Hz)", ytitle="PSD (V²/Hz)"
        )
