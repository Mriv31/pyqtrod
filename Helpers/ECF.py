...
def ECF_func(phi,winlist,i):
    ...
    for win in winlist:
        ...
        ftl_res+=np.abs(np.sum(np.exp(1.j*phi[start:stop]*i)))
    return ftl_res,i

def ECF_simple(phi,nstepmax=1000,nwindow=100,start=0,nshift=50,modemin=1,modemax=50):
...
def processInput(phir,winlist,i):
    ...
    for win in winlist:
        start, stop = win[0],win[1]
        ftl_res+=np.abs(np.sum(np.exp(1.j*phir[start:stop]*i)))
    return ftl_res,i
