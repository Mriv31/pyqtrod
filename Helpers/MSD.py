...
def draw_msd_partial(zo,start,stop,lagmin,lagmax,timestep=4e-6,dr=0,slopec=0.3,fit_tail=0,end=5,sd=0):
    ...
    if (fit_tail):
        ...
        plt.plot(timestep*lags[-100:],np.exp(a*np.log(timestep*lags[-100:])+b),c="red")

    if (dr):
        # Plot MSD
        window_size = 40
        poly_order = 4
        smooth = savgol_filter( np.log(msd_values), window_size,poly_order,deriv=1,delta=np.log(lags[-1])-np.log(lags[-2]))#[window_size:-window_size]
        try:
            ind = np.where((smooth>slopec) & (timestep*lags > 1e-4))[0][0]
            arrow_point = (timestep*lags[ind], msd_values[ind])  # Coordinates of the point to which the arrow points
            arrow_text = 'Corner'
            plt.annotate(arrow_text, arrow_point, textcoords="offset pixels", xytext=(-40, 40), ha='center', fontsize=10,
        ...
        if (sd):
            ...
            plt.figure()
            plt.plot(np.log10(timestep*lags), d2y_dx2, label='Second derivative', color='red')
...
def draw_msd_all(zo,lagmin,lagmax,loglag=0,timestep=4e-6):
    ...
    # Plot MSD
    if (loglag):
        plt.plot(np.log(lags/lags[0]), (msd_values), marker='o', linestyle='-', color='b')
        plt.yscale("linear")
        plt.xscale("linear")
